            ...
            {isPast ? (
              <div className="rounded-xl bg-red-100 border border-red-200 px-4 py-3.5">
                ...
                <p className="text-[13px] text-red-600 mt-1 leading-relaxed">
                  The {responseDays}-day notice window expired {Math.abs(daysRemaining)} day{Math.abs(daysRemaining) !== 1 ? 's' : ''} ago. Contact a legal aid attorney immediately â you may still have options in court.
                </p>
              </div>
            ) : (
              <>
...
export default function ResultCard({ result, onReset, resultMeta }) {
  ...
  async function handleShare() {
    const actions = result.tenantActions?.slice(0, 3).map(a => `â¢ ${a}`).join('\n') || ''
    const text = `My tenant rights in ${stateName}:\n\n${result.summary}\n\nKey steps:\n${actions}\n\nGenerated by invRent`
    try {
      if (navigator.share) {
        await navigator.share({ title: `Tenant Rights â ${stateName}`, text })
      } else {
  ...
  const contextLabel = [
    stateName,
    situations.length > 0
      ? situations.map(s => SITUATION_SHORT[s] || s).join(' Â· ')
      : null,
