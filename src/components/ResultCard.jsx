            ...
            {isPast ? (
              <div className="rounded-xl bg-red-100 border border-red-200 px-4 py-3.5">
                ...
                <p className="text-[13px] text-red-600 mt-1 leading-relaxed">
                  The {responseDays}-day notice window expired {Math.abs(daysRemaining)} day{Math.abs(daysRemaining) !== 1 ? 's' : ''} ago. Contact a legal aid attorney immediately Ã¢ÂÂ you may still have options in court.
                </p>
              </div>
            ) : (
              <>
...
export default function ResultCard({ result, onReset, resultMeta }) {
  ...
  async function handleShare() {
    const actions = result.tenantActions?.slice(0, 3).map(a => `Ã¢ÂÂ¢ ${a}`).join('\n') || ''
    const text = `My tenant rights in ${stateName}:\n\n${result.summary}\n\nKey steps:\n${actions}\n\nGenerated by invRent`
    try {
      if (navigator.share) {
        await navigator.share({ title: `Tenant Rights Ã¢ÂÂ ${stateName}`, text })
      } else {
  ...
  const contextLabel = [
    stateName,
    situations.length > 0
      ? situations.map(s => SITUATION_SHORT[s] || s).join(' ÃÂ· ')
      : null,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="animate-fade-in flex items-center gap-2.5 py-1">
        ...
        {contextLabel && (
          <>
            <span className="text-slate-300 select-none">ÃÂ·</span>
      ...
      <div className="rounded-2xl border border-slate-100 bg-slate-50 px-5 py-4 animate-slide-up" style={{ animationDelay: '120ms' }}>
        <p className="text-[11px] leading-relaxed text-slate-400">
          <strong className="text-slate-500">General info only, not legal advice.</strong> Laws change Ã¢ÂÂ double-check with a local attorney or legal aid clinic before taking action.{' '}
